...
export default function LoginPage() {
  ...
  if (bloqueado) {
    alert('Tu cuenta ha sido bloqueada por múltiples intentos fallidos. Contacta al administrador.');
    return;
  }
  ...
  localStorage.setItem('user_id', userData.id_persona);
  localStorage.setItem('user_nombre', userData.nombre);
  localStorage.setItem('user_apellido', userData.apellido);
  localStorage.setItem('user_qr', userData.qr_url);
  localStorage.setItem('user_dni', userData.dni);
  localStorage.setItem('user_correo', userData.correo);
  localStorage.setItem('user_uid_tarjeta', userData.uid_tarjeta);
  ...
  const manejarIntentoFallido = async () => {
    ...
    if (nuevosIntentos >= 3) {
      ...
      // Actualizar estado bloqueado en Supabase
      ...
      if (errorUpdate) {
        console.error('Error al bloquear usuario:', errorUpdate.message);
        alert('Error interno al bloquear la cuenta, intenta más tarde.');
        return;
      }

      alert('Has excedido el número de intentos. Tu cuenta ha sido bloqueada.');

      // Enviar correo a seguridad para desbloqueo
      try {
        ...
        if (response.ok && result.success) {
          console.log('Notificación enviada a seguridad.');
        } else {
          console.error('Error enviando notificación a seguridad:', result.error || 'Error desconocido');
        }
      } catch (error) {
        console.error('Error inesperado enviando notificación:', error.message);
      }
    }
  };
