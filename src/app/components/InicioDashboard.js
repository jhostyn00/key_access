...
export default function PanelGeneral() {
 ...
 useEffect(() => {
  ...
  const fetchDatos = async () => {
   ...
   // 2️⃣ Entradas y salidas (TODOS)
const { data: accesosTodos } = await supabase
  .from('acceso')
  .select('*')
  .in('tipo_movimiento', ['ingreso', 'salida']); // filtra solo esos dos tipos

const entradas = accesosTodos?.filter((a) => a.tipo === 'ingreso')?.length || 0;
const salidas = accesosTodos?.filter((a) => a.tipo === 'salida')?.length || 0;
