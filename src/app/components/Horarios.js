...
export default function AccesoVisitaPage() {
  ...
  useEffect(() => {
    const cargarVisitantesRelacionados = async () => {
  try {
    ...
    const userId = parseInt(userIdStr, 10); // Asumo que es entero, cambia si es otro tipo

    const { data, error } = await supabase
      .from('visitante')
      .select('id_visitante, persona(nombre, apellido), id_residente_visitado')
      .eq('id_residente_visitado', userId);

    if (error) throw error;

    const formatted = data.map((v) => ({
      id_visitante: v.id_visitante.toString(),
      nombre: v.persona ? `${v.persona.nombre} ${v.persona.apellido}` : 'Sin nombre',
    }));

    setVisitantes(formatted);
    setFilteredVisitantes(formatted);
  } catch (err) {
    console.error('Error al cargar visitantes relacionados:', err);
  }
};

cargarVisitantesRelacionados();
}, []);
